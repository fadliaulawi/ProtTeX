...
def download_rcsb_full(out_dir: Path, batch_size=1000):
    ...
    count_response = count_query.json()
    total = count_response.get('total_count', 0)
    ...
    while start < total:
        ...
        print(f"\nBatch {start//batch_size + 1}: {start:,} - {start+current_batch_size:,} (of {total:,})")
        ...
        r = requests.post("https://search.rcsb.org/rcsbsearch/v2/query", json={
            ...
            "return_type": "entry",
            "request_options": {"paginate": {"start": start, "rows": current_batch_size}}
        })

        response_data = r.json()
        if r.status_code != 200:
            print(f"  ✗ Error: HTTP {r.status_code} - {response_data}")
            break

        pdb_ids = [x['identifier'] for x in response_data.get('result_set', [])]
    ...
    try:
        ...
        for pfile in proteome_files:
            ...
            if result.returncode == 0 and local_path.exists():
                ...
                import tarfile
                try:
                    extract_dir = af_dir / pfile.replace('.tar', '')
                    with tarfile.open(local_path, 'r') as tar:
                        tar.extractall(extract_dir)
                    
                    # Delete the tar file after extraction
                    local_path.unlink()
                    print(f"    ✓ Extracted and deleted tar")
                    
                    # Process extracted files: un-gzip only model_v4 files, delete others
                    print(f"    Processing files (un-gzipping model_v4 only)...")
                    model_files = list(extract_dir.glob("*_model_v4.cif.gz"))
                    for gz_file in model_files:
                        try:
                            # Un-gzip the file
                            ungz_path = gz_file.with_suffix('')  # Remove .gz extension
                            ...
                            print(f"    ✗ Error un-gzipping {gz_file.name}: {e}")
                    
                    # Delete all other files (confidence and predicted_aligned_error)
                    for other_file in extract_dir.glob("*"):
                        if other_file.is_file() and not other_file.name.endswith('_model_v4.cif'):
                            try:
                                other_file.unlink()
                            except Exception as e:
                                print(f"    ✗ Error deleting {other_file.name}: {e}")
                    
                    total_downloaded += 1
                    print(f"    ✓ Processed {len(model_files)} model files")
                except Exception as e:
