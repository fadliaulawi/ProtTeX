#!/usr/bin/env python3
"""
End-to-End Inference: Protein Sequence + Question → Function Prediction (Habdine)

Habdine version: ESMFold for structure tokens, QA/metadata input, hardcoded data/habdine paths.
- Structure: ESMFold per-residue → k-means codebook → structure tokens. Sequence: ESM-2 mean-pooled.
- Input: JSON list of {QA: {sequence, question, answer}, metadata: {id, type, subset, ...}}
- Models: data/habdine/clip_alignment, data/habdine/lora, data/codebooks (shared)

Pipeline: ESM-2 (seq) + ESMFold (struct) → k-means → CLIP alignment → LoRA model → text.
"""
...
from transformers import AutoTokenizer, AutoModelForCausalLM, EsmModel, EsmForProteinFolding
...
class ProteinFunctionInference:
    ...
    def __init__(self, 
        ...
        # Load ESMFold for structure (per-residue → k-means)
        print("\n📥 Loading ESMFold (structure)...")
        self.esmfold_model_name = "facebook/esmfold_v1"
        self.esmfold_tokenizer = AutoTokenizer.from_pretrained(self.esmfold_model_name)
        self.esmfold_model = EsmForProteinFolding.from_pretrained(self.esmfold_model_name).to(self.device)
    ...
    def _extract_esm_embeddings(self, sequence: str):
        """ESM-2 mean-pooled sequence embedding [2560]."""
        inputs = self.esm_tokenizer([sequence], return_tensors="pt", padding=False)
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        with torch.no_grad():
            out = self.esm_model(**inputs, output_hidden_states=True)
        h = out.hidden_states[-1][0, 1:-1, :].cpu().numpy()
        return h.mean(axis=0)
    
    def _extract_esmfold_embeddings(self, sequence: str):
        """ESMFold trunk per-residue embeddings [seq_len, D] for structure tokens."""
        inputs = self.esmfold_tokenizer([sequence], return_tensors="pt", padding=False, truncation=False)
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        with torch.no_grad():
            out = self.esmfold_model.esm(**inputs, output_hidden_states=True)
    ...
    def generate(self, 
        ...
        """
        if self.alignment_model is None or self.esm_model is None or self.esmfold_model is None or self.kmeans is None:
            raise ValueError("Tri-modal generation requires alignment_model, esm_model, esmfold_model, and kmeans")
        sequence = sequence[:1024]  # Truncate for memory
        
        # Sequence embedding: ESM-2 mean-pooled
        seq_emb = self._extract_esm_embeddings(sequence)  # [2560]
        
        # Structure tokens: ESMFold per-residue → k-means → token offset
        esmfold_emb = self._extract_esmfold_embeddings(sequence)
        cluster_ids = self.kmeans.predict(esmfold_emb)
    ...
    def generate_plain_llama_with_structure(self,
        ...
        Returns:
            Generated function text
        """
        if self.esmfold_model is None or self.kmeans is None:
            raise ValueError("Structure tokens require esmfold_model and kmeans")
        
        sequence = sequence[:1024]  # Truncate for memory
        
        # Structure: ESMFold per-residue → k-means → space-separated cluster IDs
        esmfold_emb = self._extract_esmfold_embeddings(sequence)
        cluster_ids = self.kmeans.predict(esmfold_emb)
        structure_str = " ".join([str(int(cid)) for cid in cluster_ids[:512]])
    ...
    def generate_plain_llama_with_embeddings(self,
        ...
        """
        if self.alignment_model is None or self.esm_model is None or self.esmfold_model is None or self.kmeans is None:
            raise ValueError("Embeddings require alignment_model, esm_model, esmfold_model, and kmeans")
        
        sequence = sequence[:1024]  # Truncate for memory
        
        # Sequence: ESM-2 mean-pooled. Structure: ESMFold per-residue → k-means
        seq_emb = self._extract_esm_embeddings(sequence)
        esmfold_emb = self._extract_esmfold_embeddings(sequence)
        cluster_ids = self.kmeans.predict(esmfold_emb)
        structure_tokens = torch.tensor(
            [self.token_offset + int(cid) for cid in cluster_ids],
...
def main():
    ...
    parser.add_argument('--input', type=str, required=True, help='Input JSON: list of {QA: {sequence, question, answer}, metadata: {id, type, subset, ...}}')
    ...
    # Paths (habdine: data/habdine for models; data/codebooks shared)
    lora_path = Path('data/habdine/lora') / model_config.output_dir_suffix / f'K{k_clusters}' / f'final_lora_K{k_clusters}'
    alignment_model_path = Path('data/habdine/clip_alignment') / f'{args.model}_K{k_clusters}' / f'best_model_K{k_clusters}.pt'
    codebook_path = Path('data/codebooks') / f'structure_codebook_K{k_clusters}.pkl'
    ...
    if not args.test_base_only:
        if not lora_path.exists():
            ...
            print(f"   Run 03b_train_lora.py first with --model {args.model}!")
    ...
    if not alignment_model_path.exists():
        ...
        print(f"   Run 03a_train_clip_alignment.py first with --model {args.model}!")
        return
    
    if not codebook_path.exists():
        print(f"❌ Codebook not found: {codebook_path}")
        print(f"   Run script 02_train_kmeans_codebook.py (or main pipeline) first!")
        return
    
    # Load inputs: QA and metadata format {QA: {sequence, question, answer}, metadata: {id, type, subset, ...}}
    print(f"\n📥 Loading inputs from file: {args.input}")
    with open(args.input, 'r') as f:
        input_data = json.load(f)
    
    if not isinstance(input_data, list):
        print(f"❌ Input must be a JSON list of objects with 'QA' and 'metadata'")
        return
    
    inputs = []
    for item in input_data:
        if not isinstance(item, dict) or 'QA' not in item or 'metadata' not in item:
            print(f"❌ Each item must have 'QA' and 'metadata' (QA: sequence, question, answer; metadata: id, type, subset, ...)")
            return
        qa = item.get('QA', {})
        ...
        inputs.append({
            ...
            'ground_truth_function': qa.get('answer', ''),
