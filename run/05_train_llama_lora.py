...
class TriModalLlamaTrainer:
    ...
    def __init__(self, 
        ...
        """
        Args:
            llama_model: Llama model with LoRA (DDP wrapped)
        ...
        self.base_model = llama_model.module if isinstance(llama_model, DDP) else llama_model
        ...
        self.ema_loss = None
        self.ema_decay = 0.99  # Smoothing factor for loss (original version)
    ...
    def construct_prompt_with_embeddings(self, batch):
        ...
        Questions vary by task type (PFUD, PSAD, PDD, PSPD).
        ...
        for i in range(batch_size):
            ...
            # Use Llama 3.1 Instruct chat template format
            # Part 1: User message header + question + sequence + embedding placeholder
            ...
            # Format: <|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n
            ...
            # Part 4: Target (function) + end of turn
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if (batch_idx + 1) % self.gradient_accumulation_steps == 0:
                ...
                if self.rank == 0:  # Only log on main process
                    ...
                    wandb.log({
                        'train/batch_loss': raw_loss,  # Raw loss (noisy but informative)
                        'train/batch_loss_smooth': self.ema_loss,  # Smoothed loss (stable)
...
def main():
    ...
    if rank == 0:
        print("=" * 70)
        print("TRAIN LLAMA-3.1 WITH LORA: Protein Function Prediction (DDP)")
        print("=" * 70)
    ...
    # Fixed hyperparameters - optimized for H200 140GB GPUs (aligned with DataParallel)
    batch_size = 6  # Per GPU (8 * 8 = 64 total samples in parallel)
    gradient_accumulation_steps = 4  # Balance between stability and speed (8 was too slow)
    ...
    if rank == 0:
        ...
        wandb.init(
            ...
            name=f"run-lora-ddp-{timestamp}",
            config={
                'k_clusters': k_clusters,
                'epochs': args.epochs,
                'batch_size_per_gpu': batch_size,
                'gradient_accumulation_steps': gradient_accumulation_steps,
                'effective_batch_size': batch_size * gradient_accumulation_steps * world_size,  # 8 * 4 * 8 = 256
                'num_gpus': world_size,
    ...
    output_dir = Path(f'data/llama_lora/K{k_clusters}_test{args.test_batches}')
    ...
    llama = DDP(llama, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)
    
    if rank == 0:
        print("✅ Llama loaded with LoRA and DDP")
    ...
    if rank == 0:
        ...
        print(f"   Effective batch size: {batch_size * gradient_accumulation_steps * world_size} (increased for stability)")
    ...
    for epoch in range(args.epochs):
        ...
        # Note: scheduler.step() is called per training step
