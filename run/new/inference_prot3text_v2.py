"""Inference script for V2 enhanced architecture with cross-attention + gated fusion."""
import os
...
def inference_on_device(rank: int, world_size: int, args: Dict[str, Any]):
    ...
    # V2 Enhanced fusion components
    cross_attention_fusion = CrossAttentionFusion(
        hidden_dim=llama_decoder.config.hidden_size,
        num_heads=args.get("fusion_num_heads", 8),
        dropout=args.get("fusion_dropout", 0.1)
    )
    cross_attention_fusion.to(torch.float16)
    
    gated_fusion = GatedFusion(
        hidden_dim=llama_decoder.config.hidden_size,
        gate_hidden_dim=args.get("gate_hidden_dim", 256),
        dropout=args.get("fusion_dropout", 0.2)
    )
    gated_fusion.to(torch.float16)

    if rank == 0:
        print(f"✅ V2 Fusion initialized: CrossAttention({args.get('fusion_num_heads', 8)} heads) + GatedFusion")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Protein-to-text inference with V2 enhanced ESM-LLAMA model")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    args = {
        ...
        # V2 fusion hyperparameters (must match training config)
        "fusion_num_heads": 8,
        "fusion_dropout": 0.2,
        "gate_hidden_dim": 256,
