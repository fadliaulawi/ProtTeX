...
from .train_legacy import load_model, setup, cleanup
...
def inference_on_device(rank: int, world_size: int, args: Dict[str, Any]):
    ...
    generate_dataset = Prot2TextInstructDataset(
        ...
        max_description_length=None,  # fetch complete labels for metric computation
    )
    if args["debug_trim_generate_split"]:
        generate_dataset.usable_file_names = generate_dataset.usable_file_names[:args["debug_trim_generate_split"]]
    generate_sampler = DistributedSampler(generate_dataset, rank=rank, num_replicas=world_size, shuffle=False)
    generate_loader = Prot2TextDerivedDataLoader(
        generate_dataset,
        mode="inference", 
...
if __name__ == "__main__":
    ...
    if parsed_args.save_generation_postfix_identifier: 
        parsed_args.save_generation_postfix_identifier = f"""
            {start_timestamp}_[{parsed_args.save_generation_postfix_identifier}]
        """
    else: 
