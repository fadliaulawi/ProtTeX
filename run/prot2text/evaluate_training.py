...
generation_path = "data/prot2text/generations/generations_260217_191145"
generation_id = "generation_260218_084325"

num = 0
data = {}
for file in os.listdir(generation_path):
    if file.startswith(generation_id) and file.endswith(".json"):
        with open(os.path.join(generation_path, file), "r") as f:
            file_data = json.load(f)
            data.update(file_data)
            num += len(file_data)

print(f"Loaded {num} examples = {len(data)} generations.")
predictions = [item["pred"] for item in data.values()]
references = [item["true"] for item in data.values()]
...
with open("data/prot2text/evaluation_results.json", "w") as f:
