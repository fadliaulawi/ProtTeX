...
def load_model(args: Dict[str, Any]) -> PeftModel:
    ...
    # CHANGES
    for p in model.esm_encoder.parameters():
...
def train_on_device(
    ...
    if args["debug_trim_train_split"]:
        train_dataset.data = train_dataset.data.sample(n=min(args["debug_trim_train_split"], len(train_dataset.data)), random_state=args["random_seed"])
    ...
    if args["debug_trim_eval_split"]:
        eval_dataset.data = eval_dataset.data.sample(n=min(args["debug_trim_eval_split"], len(eval_dataset.data)), random_state=args["random_seed"])
    ...
    # optimizer = AdamW(model.parameters(), lr=args["learning_rate"], eps=1e-6, betas=(0.9, 0.999), weight_decay=0.01)
...
if __name__ == '__main__':
    ...
    # PARAMS
    parsed_args.random_seed = 42
    parsed_args.save_checkpoint_dir = "data/prot2text/checkpoints"
    parsed_args.esm_path = "facebook/esm2_t36_3B_UR50D"
    parsed_args.llama_path = "meta-llama/Llama-3.1-8B-Instruct"
    parsed_args.torch_dtype = torch.bfloat16
    parsed_args.root_csv_dir = "data/csv"
    parsed_args.train_split = "train"
    parsed_args.eval_split = "validation"
    parsed_args.batch_size_per_device = 4
    parsed_args.gradient_accumulation_steps = 8
    parsed_args.num_epochs = 24
    parsed_args.save_every_epochs = 1
    parsed_args.learning_rate = 5e-5
    parsed_args.scheduler_gamma = 0.95
    parsed_args.lora_rank = 32
    parsed_args.include_text_fields = True
    parsed_args.name_dropout = 0.1
    parsed_args.taxonomy_dropout = 0.1

    parsed_args.debug_trim_train_split = None
    parsed_args.debug_trim_eval_split = None
