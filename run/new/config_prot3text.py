...
class FusionLayer(torch.nn.Module):
    """Learnable weighted fusion of sequence and structure embeddings."""
    def __init__(self, alpha_init: float = 0.5):
        super().__init__()
        # Use logit parameterization for stability
        # sigmoid(0) = 0.5, so initialize to log(alpha_init / (1 - alpha_init))
        import math
        init_logit = math.log(alpha_init / (1 - alpha_init + 1e-8))
        self.alpha_logit = torch.nn.Parameter(torch.tensor(init_logit))
    
    def forward(self, seq_emb: torch.FloatTensor, struct_emb: torch.FloatTensor) -> torch.FloatTensor:
        alpha = torch.sigmoid(self.alpha_logit)
        return alpha * seq_emb + (1 - alpha) * struct_emb
...
class Esm2LlamaInstructForCausalLM(PreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if config is not None:
            ...
            self.fusion_layer = FusionLayer(alpha_init=config.structure_alpha)
            self.llama_decoder = LlamaForCausalLM(config.llama_config)
            # Note: esmfold_encoder and codebook_centroids must be set externally
    ...
    def forward(
        ...
        """Forward pass through dual encoders (ESM2 + ESMFold), adapters, fusion, and LLaMA decoder."""
        ...
        seq_hidden_states = seq_encoder_output.last_hidden_state  # (bsz, seq_len, 2560)
        encoder_attention_mask = protein_attention_mask
        if return_encoder_outputs:
            return seq_encoder_output
        
        # === STRUCTURE PATH: ESMFold encoder + K-means ===
        with torch.no_grad():  # ESMFold is frozen
            struct_encoder_output = self.esmfold_encoder.esm(
                input_ids=protein_input_ids,
                attention_mask=protein_attention_mask,
                output_hidden_states=True
            )
            struct_hidden_states = struct_encoder_output.hidden_states[-1]  # (bsz, seq_len, 2560)
        
        # K-means inference: convert embeddings to cluster centroids
        if self.codebook_centroids is not None:
            bsz, seq_len, emb_dim = struct_hidden_states.shape
            # Flatten batch dimension for k-means
            struct_flat = struct_hidden_states.view(-1, emb_dim)  # (bsz*seq_len, 2560)
            
            # Compute distances to all centroids
            centroids = self.codebook_centroids.to(struct_flat.device)  # (K, 2560)
            distances = torch.cdist(struct_flat.unsqueeze(0), centroids.unsqueeze(0)).squeeze(0)  # (bsz*seq_len, K)
            cluster_ids = torch.argmin(distances, dim=1)  # (bsz*seq_len,)
            
            # Lookup centroids
            struct_hidden_states = centroids[cluster_ids].view(bsz, seq_len, emb_dim)  # (bsz, seq_len, 2560)
        ...
        # === FUSION ===
        fused_embeddings = self.fusion_layer(seq_projected, struct_projected)  # (bsz, seq_len, 4096)
    ...
    def generate(
        ...
        **kwargs
        ...
        """Generate text from fused protein embeddings (sequence + structure)."""
